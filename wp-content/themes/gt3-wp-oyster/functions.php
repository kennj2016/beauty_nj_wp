    ...
    <div id="comment-<?php comment_ID(); ?>" class="stand_comment">
        ...
        <div class="thiscommentbody">
            <div class="comment_info">
                <h6 class="author_name"><?php printf('%s', get_comment_author_link()) ?> <?php edit_comment_link('(Edit)', '  ', '') ?></h6>
                <h6 class="date"><?php printf('%1$s', get_comment_date("F d, Y")) ?></h6>
...
function get_pf_type_output($args)
{
    ...
    if (isset($pf)) {
        ...
        ";
			...
			$compile .= '<div class="pf_audio_container blog_post_image wrapped_audio">' . $gt3_theme_pagebuilder['post-formats']['audiourl'] . '</div>';
		} else {
            if (isset($fw_post) && $fw_post == true) {
                ...
                {
                    preg_match('/<img(.*?)src=(\'|")(.*?)(\'|")(.*?)>/is', get_the_content(), $thumb);
                    if(!empty($thumb[3])){
                        if (isset($link))
                            $compile .= '<a href="'. $link .'">';       
                        $compile .= '<img class="featured_image_standalone" src="' . $thumb[3] . '" alt="" />';
                        if (isset($link))
                            $compile .= '</a>';
                    }
                }
                ...
                else
                {
                    ...
                    if(!empty($thumb[3])){
                        if (isset($link))
                            $compile .= '<a href="'. $link .'">';
                        $compile .= '<img class="featured_image_standalone" src="' . aq_resize($thumb[3], $width, $height, true, true, true) . '" alt="" />';
                        if (isset($link))
                            $compile .= '</a>';
                    }
                }
...
function gt3_get_field_media_and_attach_id($name, $attach_id, $previewW = "200px", $previewH = null, $classname = "")
{
    return "<div class='select_image_root " . $classname . "'>
		<br><h4>". __('Please select image to display as a featured one', 'theme_localization')."</h4>
